...
To ensure the value of the statistical analysis across usergroups and rounds, the underlying knapsack problems are designed to be sufficiently similar. For this purpose, a benchmark is introduced to compare the difficulty of knapsack problems and to design equally difficult problems.\\
...
\subparagraph{Comparability across usergroups and rounds}
...
In order to compare the performance of participants in \textbf{different usergroups}, the underlying knapsack problems are the same for each individual round.
...
Each usergroup is assigned 4 rounds with 100 boxes per round. The first round is the Trial round\footnote{No data was conducted from the Trial round.} while the other 3 rounds are shown during the actual game.
...
The colour scale is presented individually per usergroup to the participant on the left of the interface (Figure \ref{fig:Interface}).
...
\caption{Exemplary interface (Usergroup 5, Trial 1 )}
\end{center}
\label{fig:Interface}
\end{figure} 
...
When arriving at the page, the program creates an individual user who is randomly assigned to one usergroup and an individual code is generated. 
...
This section aims to provide a descriptive overview of the unfiltered data. First, the usergroup statistics are introduced, second the descriptive results from the game are analysed and third the answers from the participants are examined.
%% ===============================
\paragraph{Usergroups and payout}
\label{ch:Evaluation:sec:DescriptiveStatistics:subsec:Distributionofusergroups}

The participants were randomly and uniformly assigned to one usergroup once they directed to the welcome page. The low share of usergroup 1 is related to the fact that the usergroup was added in Trial 2 (Table \ref{DistributionofUsergroups}).\\ 
\begin{table}[htbp] % Distribution of Usergroups
  \centering
  \caption{Distribution of Usergroups}
    \label{DistributionofUsergroups}
    \begin{tabular}{ccc|c|c}
    \toprule
    Usergroup & Users & Share & Share of Dropouts\footnotemark & Share of Minimum Payout\\
    \midrule
    1     & 33    & 10\%  & 37\% & 31\%\\
    2     & 88    & 25\%  & 40\% & 25\%\\
    3     & 78    & 22\%  & 40\% & 31\%\\
...
The distribution of payout presented in Figure \ref{Distributionofpayout} shows that the medians of the payout per usergroup is decreasing with the \textit{NumberOfColours} in Trial 1, whereas an inverted U-Shape can be detected for the medians in Trial 2, with a peak at the medium 7-colours level.\\
...
\begin{center} 
  \caption{Distribution of payout among usergroups for each Trial}
...
\begin{subfigure} 
\centering
\includegraphics[height = 0.38\textwidth]{HistogramFinalResult.pdf}
\end{subfigure} 
\begin{subfigure} 
\centering
  \includegraphics[height = 0.38\textwidth]{BoxplotFinalResult.pdf}
\end{subfigure}
  \caption{FinalResult - Histogram and Box plot}
    \label{DistributionFinalResult} 
\end{center}
\end{figure}
The distribution of the different result types among different usergroups is exemplary, as shown in Figure \ref{DistributionFinalResult} (right) for the \textit{FinalResult} and indicates a decreasing interquartile range with an increasing number of colours. The medians for all usergroups are above the bonus bar and show a tendency to form a U-Shape with a peak at the treatment with three colours.\\
Significant outliers can be found in all usergroups except for the usergroup with two colours. Outliers with a \textit{FinalResult} lower than 20\% can be explained by a lack of effort and understanding on behalf of the participant, as opposed to an experience of information overload. A result greater than this value can namely be achieved by simply clicking on random boxes to fill up the knapsack.
...
The box plot of \textit{FirstTime} shows similar medians across usergroups. The interquartile range is smaller for usergroups 2 and 4, and bigger for usergroup 3. The standard deviation is the lowest for all time types.\\
The interquartile range is similar among all usergroups for the \textit{BestTime}. The values for the medians define a U-shape, with usergroup 2 as the apex. The medians for \textit{FinalTime} vary across usergroups, yet the interquartile range does not seem to be dependent on the number of colours.\\

\begin{figure}[htbp] % DistributionFinalTime
\begin{center} 
\begin{subfigure} 
\centering
\includegraphics[height = 0.38\textwidth]{HistogramFinalResultTime.pdf}
\end{subfigure} 
\begin{subfigure} 
\centering
  \includegraphics[height = 0.38\textwidth]{BoxplotFinalResultTime.pdf}
\end{subfigure}
...
The box plots of \textit{DecisionTime} show a wide range, from below 0s to up to 6s for all usergroups. The medians are similar and usergroup 3 has a wider interquartile range. 

%% ===============================
\paragraph{Questionnaire}
\label{ch:Evaluation:sec:DescriptiveStatistics:subsec:Questionnaire}

Figure \ref{ProfilePlot} summarizes the medians for all the answers per usergroup. In general, the medians for all usergroups show similar values, so the different treatments do not seem to have an influence on the answers.\\
...
There are no significant signs for negative emotions like insecurity and irritation triggered by the experiment, since all usergroups indicate a low level of negative emotions. 
...
\begin{subfigure} 
\centering
 \includegraphics[height = 0.55\textwidth]{ProfilePlotLegend.pdf}
\end{subfigure} 
\begin{subfigure}
\centering
 \includegraphics[height = 0.55\textwidth]{ProfilePlotAnswer.pdf}
\end{subfigure}   
